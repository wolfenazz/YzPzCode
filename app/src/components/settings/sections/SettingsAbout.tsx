...
const ExternalLinkIcon = () => (
  <svg className="w-3 h-3 text-[var(--text-secondary)] group-hover:text-[var(--accent)] opacity-50 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
  </svg>
);

const GitHubIcon = () => (
  <svg className="w-4 h-4 text-[var(--text-secondary)] group-hover:text-[var(--accent)] opacity-60 transition-colors" fill="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <div className="space-y-8 font-mono">
      ...
      <div className="space-y-4">
        <div className="bg-[var(--bg-secondary)]/80 border border-[var(--border-primary)] backdrop-blur-sm rounded-lg p-5">
          <div className="flex items-center gap-4">
            <div className="w-16 h-16 rounded-lg bg-[var(--bg-primary)]/60 border border-[var(--border-primary)]/70 flex items-center justify-center p-1.5">
        ...
        <div className="bg-[var(--bg-secondary)]/80 border border-[var(--border-primary)] backdrop-blur-sm rounded-lg p-5 space-y-4">
          ...
          <div className="space-y-2">
            {AUTHORS.map((author) => (
              <div
                key={author.name}
                className="flex items-center justify-between px-4 py-3 bg-[var(--bg-primary)]/30 border border-[var(--border-primary)]/20 rounded-lg hover:border-[var(--accent-border)] transition-colors duration-150"
              >
                <p className="text-xs text-[var(--text-primary)] font-medium">{author.name}</p>
                <p className="text-[10px] text-[var(--text-secondary)]">{author.role}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-[var(--bg-secondary)]/80 border border-[var(--border-primary)] backdrop-blur-sm rounded-lg p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            {TECH_STACK.map((tech) => (
              <div
                key={tech.name}
                className="flex items-center gap-2.5 px-3 py-2.5 bg-[var(--bg-primary)]/30 border border-[var(--border-primary)]/20 rounded-lg"
              >
                <div className="w-4 h-4 flex items-center justify-center flex-shrink-0">
                  {tech.icon}
                </div>
                <div className="min-w-0">
                  <p className="text-xs text-[var(--text-primary)] font-medium leading-tight">{tech.name}</p>
                  <p className="text-[9px] text-[var(--text-secondary)] leading-tight mt-0.5">{tech.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="bg-[var(--bg-secondary)]/80 border border-[var(--border-primary)] backdrop-blur-sm rounded-lg p-5 space-y-4">
          ...
          <div className="space-y-2">
            <a
              ...
              className="flex items-center justify-between px-4 py-3 bg-[var(--bg-primary)]/30 border border-[var(--border-primary)]/20 rounded-lg hover:border-[var(--accent-border)] transition-all duration-150 group"
            >
            ...
            >
              <div className="flex items-center gap-3">
                <svg className="w-4 h-4 text-[var(--text-secondary)] group-hover:text-[var(--accent)] opacity-60 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="bg-[var(--bg-secondary)]/80 border border-[var(--border-primary)] backdrop-blur-sm rounded-lg p-5 space-y-3">
          <h3 className="text-xs font-mono font-bold text-[var(--accent-text)] uppercase tracking-[0.2em]">
            License
          </h3>
