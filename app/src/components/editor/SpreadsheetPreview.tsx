...
const SpreadsheetPreviewInner: React.FC<SpreadsheetPreviewProps> = ({ filePath, fileName }) => {
  ...
  return (
    <div className="absolute inset-0 flex flex-col overflow-hidden bg-[#262626]">
      ...
      <div className="flex-1 overflow-auto min-h-0">
        {!loading && currentSheet && (
          <table className="w-full border-collapse spreadsheet-table spreadsheet-dark">
            <thead className="sticky top-0 z-10 bg-zinc-900">
              <tr>
                <th className="px-2 py-1 text-[9px] font-mono text-center w-12 shrink-0 border-r border-b text-zinc-600 border-zinc-800 bg-zinc-950/80">
                  #
                </th>
                {Array.from({ length: currentSheet.colCount }, (_, i) => (
                  <th
                    key={i}
                    className="px-2 py-1 text-[9px] font-mono text-center font-semibold border-r border-b min-w-[80px] text-zinc-500 border-zinc-800 bg-zinc-950/80"
                  >
            ...
            <tbody>
              {currentSheet.rows.map((row, rowIdx) => (
                <tr key={rowIdx} className="transition-colors hover:bg-zinc-900/50">
                  <td className="px-2 py-1 text-[9px] font-mono text-center border-r border-b text-zinc-700 border-zinc-800 bg-zinc-950/50">
                    {rowIdx + 2}
                  </td>
                    ...
                    return (
                      <td
                        ...
                        className={`px-2 py-1 text-[11px] font-mono border-r border-b truncate max-w-[200px] ${
                          isHeaderRow
                            ? 'font-semibold text-zinc-300 border-zinc-800'
                            : isNumeric
                              ? 'text-right text-blue-400 border-zinc-800'
              ...
              {currentSheet.rows.length === 0 && (
                <tr>
                  <td
                    ...
                    className="px-4 py-8 text-center text-[10px] uppercase tracking-widest text-zinc-600"
                  >
                    Empty sheet
                  </td>
                </tr>
