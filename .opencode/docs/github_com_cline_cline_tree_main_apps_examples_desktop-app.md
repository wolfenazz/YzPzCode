# cline/apps/examples/desktop-app at main · cline/cline · GitHub

> Source: https://github.com/cline/cline/tree/main/apps/examples/desktop-app
> Cached: 2026-08-13T09:55:36.254Z
...
[`sidecar/shell-path.ts`](/cline/cline/blob/main/apps/examples/desktop-app/sidecar/shell-path.ts).
## Web Visual System

[](#web-visual-system)
The framework-neutral color, typography, radius, and navigation contract lives
in the internal [`@cline/ui`](/cline/cline/blob/main/sdk/packages/ui/README.md) workspace
...
runtime. See [`webview/styles/README.md`](/cline/cline/blob/main/apps/examples/desktop-app/webview/styles/README.md) for the
...
Get the **Developer ID Application** identity from your team admin. A `.cer` alone is not enough — you need the private key. If the admin generated the CSR, have them export the identity from Keychain Access as a `.p12` and import it:
`security import BeeCertificates.p12 -k ~/Library/Keychains/login.keychain-db -T /usr/bin/codesign -T /usr/bin/security`
If `security find-identity -v -p codesigning` still reports `0 valid identities`, the Apple intermediate CA is missing. Install it:
...
- Re-run `security find-identity -v -p codesigning` — it should now list `Developer ID Application: <Team Name> (<TEAMID>)`. That exact quoted string is your `APPLE_SIGNING_IDENTITY`.

- Get an **App Store Connect API key** from the admin: the `AuthKey_<KEYID>.p8` file, the Key ID, and the Issuer ID (a UUID from App Store Connect → Users and Access → Integrations). This is used for notarization only — nothing is published.
...
- The React app uses `lib/desktop-client.ts` and no longer imports `@tauri-apps/api/core` directly in feature code.

- Tool approval updates are pushed from the backend instead of polled from the UI.

- Session process context resolves `workspaceRoot` from git root and uses that same path as default `cwd` for chat runtime and git operations unless explicitly overridden.
...
- [`src-tauri/src/main.rs`](/cline/cline/blob/main/apps/examples/desktop-app/src-tauri/src/main.rs) - Tauri shell lifecycle, backend launch, and native-only commands

- [`sidecar/index.ts`](/cline/cline/blob/main/apps/examples/desktop-app/sidecar/index.ts) - persistent Bun sidecar and Hub-daemon entry dispatch

- [`sidecar/chat-session.ts`](/cline/cline/blob/main/apps/examples/desktop-app/sidecar/chat-session.ts) - shared-Hub chat session adapter

- [`webview/lib/desktop-client.ts`](/cline/cline/blob/main/apps/examples/desktop-app/webview/lib/desktop-client.ts) - typed desktop websocket client

- [`webview/hooks/use-chat-session.ts`](/cline/cline/blob/main/apps/examples/desktop-app/webview/hooks/use-chat-session.ts) - UI chat session state + backend subscriptions

- [`webview/lib/chat-schema.ts`](/cline/cline/blob/main/apps/examples/desktop-app/webview/lib/chat-schema.ts) - chat message schema used by the UI

- [`webview/components/views/settings/routine-view.tsx`](/cline/cline/blob/main/apps/examples/desktop-app/webview/components/views/settings/routine-view.tsx) - Routine schedules UI
...
- Full v1 schema for the persisted messages file, including failure/retry semantics and golden fixtures, is documented in [`packages/core/docs/messages-contract-v1.md`](/cline/cline/blob/main/sdk/packages/core/docs/messages-contract-v1.md).
